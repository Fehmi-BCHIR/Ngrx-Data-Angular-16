...
export class CourseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.lessons$ = this.lessonsService.entities$.pipe(
      withLatestFrom(this.course$), // combine the two observables (course$ and lessons$)
      tap(([lessons, course]) => {
        if (this.nextPage == 0) {
    ...
    // delay the updating of the UI so the loading$ is only going to get affected in the next change detection run
  }
